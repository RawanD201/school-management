    ...
    <div class="flex justify-between items-center flex-1 lg:hidden">
        <div class="w-10">
            @if ($paginator->hasPages() && !$paginator->onFirstPage())
                <x-tables::icon-button :wire:click="'previousPage(\'' . $paginator->getPageName() . '\')'" rel="prev"
                    :icon="$previousArrowIcon" :label="__('tables::table.pagination.buttons.previous.label')" />
            @endif
        </div>
        ...
        <div class="w-10">
            @if ($paginator->hasPages() && $paginator->hasMorePages())
                <x-tables::icon-button :wire:click="'nextPage(\'' . $paginator->getPageName() . '\')'" rel="next"
                    :icon="$nextArrowIcon" :label="__('tables::table.pagination.buttons.next.label')" />
            @endif
        </div>
    </div>
        ...
        <div class="flex items-center">
            ...
            @else
                <div @class([
                    ...
                    {{ trans_choice('tables::table.pagination.overview', $paginator->total(), [
        ...
        <div class="flex items-center justify-end">
            @if ($isSimple)
                @if ($paginator->hasMorePages())
                    <x-tables::button :wire:click="'nextPage(\'' . $paginator->getPageName() . '\')'" :icon="$nextArrowIcon"
                        icon-position="after" rel="next" size="sm" color="secondary">
                                ...
                                @if (is_array($element))
                                    @foreach ($element as $page => $url)
                                        <x-tables::pagination.item
                                            ...
                                            :label="$page" :aria-label="trans_choice(
                                                'tables::table.pagination.buttons.go_to_page.label',
                                                $page,
                                                ['page' => $page],
