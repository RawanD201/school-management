...
<x-filament::dropdown placement="bottom-end">
    ...
    <x-filament::dropdown.header :color="$accountItem?->getColor() ?? 'secondary'" :icon="$accountItem?->getIcon() ?? 'heroicon-o-user-circle'" :href="$accountItemUrl" :tag="filled($accountItemUrl) ? 'a' : 'div'">
    ...
    <x-filament::dropdown.list x-data="{
        ...
        <div>
            @if (config('filament.dark_mode'))
                <x-filament::dropdown.list.item icon="heroicon-o-moon" x-show="theme === 'dark'"
                    x-on:click="close(); mode = 'manual'; theme = 'light'">
                    {{ __('filament::layout.buttons.light_mode.label') }}
                </x-filament::dropdown.list.item>

                <x-filament::dropdown.list.item icon="heroicon-o-sun" x-show="theme === 'light'"
        ...
        <x-filament::dropdown.list.item :color="$logoutItem?->getColor() ?? 'secondary'" :icon="$logoutItem?->getIcon() ?? 'heroicon-o-logout'" :action="$logoutItem?->getUrl() ?? route('filament.auth.logout')" method="post"
            tag="form">
